...
class Graph:
    ...
    def make_undirected(self):
        #luam toate nodurile de start si facem legaturile cu nodurile de end, deci de ex pun Helsinki si in dictionarul lui Oslo si si pe Oslo il pun la Helsinki
        for a in list(self.graph_dict.keys()):
            for (b, dist) in self.graph_dict[a].items():
                self.graph_dict.setdefault(b, {})[a] = dist
    #setdefault(key, value) - returneaza valoarea cheii specificate si daca nu exista cheia in dictionar o adauga cu valoarea data
    ...
    def nodes(self):
        #set cu cheile din dictionar, deci toate orasele
...
def main():
    ...
    graph.connect("Barcelona", "Lyan", 644)
